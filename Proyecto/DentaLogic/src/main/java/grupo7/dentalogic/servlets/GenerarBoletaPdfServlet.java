...
@WebServlet("/generar-boleta-pdf")
public class GenerarBoletaPdfServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            // Generar PDF
            generarPdf(response, detalle);
            
        } catch (NumberFormatException e) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "ID de empleado inválido");
        } catch (ServletException | IOException e) {
            throw new ServletException("Error al generar boleta", e);
        }
    }

    private void generarPdf(HttpServletResponse response, ResumenNomina detalle) 
            throws IOException, ServletException {
        
        response.setContentType("application/pdf");
        String filename = "boleta_" + detalle.getNombreEmpleado().replace(" ", "_") + ".pdf";
        response.setHeader("Content-Disposition", "attachment; filename=" + filename);
        
        try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
            PdfRendererBuilder builder = new PdfRendererBuilder();
            builder.withHtmlContent(generarHtmlBoleta(detalle), "/");
            builder.toStream(os);
            builder.run();
